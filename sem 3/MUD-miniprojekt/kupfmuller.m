...
dTzew = 0; % 0 lub 2
dFp = FpN*0.0; % 0 lub 0.2

% typ wykresu
styl = {'--'; ':'; '--'}; grub = [1; 2; 1]; kolo = ['r'; 'b'; 'r'];
...
p = plot(t, TS(: , i) - TS0(i));
c = p.LineWidth; p.LineWidth = grub(i);  p.LineStyle =styl{i}; p.Color = kolo(i);
xlabel('Czas [s]'); ylabel('Temepretura [C]');  hold on; grid on; grid minor;
...
kupf0 = exp(-s*T0);
kupf1 = kupf0 * (k1/(T1*s +1));
kupf2 = kupf0 * (k2/(T2*s +1));

% model 1

opcje = stepDataOptions('StepAmplitude',[dQg]);
figure(2);
step(kupf1, opcje);

% model 2

% opcje = stepDataOptions('StepAmplitude',[dTzew]);
% figure(3);
% 
% step(kupf2, opcje);
